...
class MergePurchaseOrder(models.TransientModel):
    ...
    merge_type = fields.Selection([
        ('new_cancel', 'Create new order and cancel all selected sale orders'),
        ('new_delete', 'Create new order and delete all selected sale orders'),
    ...
    def merge_orders(self):
        ...
        if len(self._context.get('active_ids', [])) < 2:
            raise UserError(_('Please select atleast two sale orders to perform '
                              'the Merge Operation.'))
        if any(order.state not in ['draft', 'sent'] for order in sale_orders):
            raise UserError(_('Please select Sale orders which are in Quotation state '
        ...
        if any(order.partner_id.id != partner for order in sale_orders):
            raise UserError(_('Please select Sale orders whose Customers are same to '
            ...
            for order in sale_orders:
                for line in order.order_line:
                    ...
                    if existing_so_line:
                        ...
                        so_taxes = [tax.id for tax in existing_so_line.tax_id]
                        [so_taxes.append((tax.id))
                         for tax in line.tax_id]
                        existing_so_line.tax_id = [(6, 0, so_taxes)]
                    else:
            ...
            for order in sale_orders:
                ...
                for line in order.order_line:
                    ...
                    if existing_so_line:
                        existing_so_line.product_uom_qty += line.product_uom_qty
                        so_taxes = [tax.id for tax in existing_so_line.tax_id]
                        [so_taxes.append((tax.id)) for tax in line.tax_id]
                        existing_so_line.tax_id = [(6, 0, so_taxes)]
                    else:
                        line.copy(default=default)
            for order in sale_orders:

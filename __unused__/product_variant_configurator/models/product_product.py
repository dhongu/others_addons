...
class ProductProduct(models.Model):
    ...
    def _get_product_attributes_values_dict(self):
        ...
        for val in res:
            value = self.attribute_value_ids.filtered(lambda x: x.attribute_id.id == val['attribute_id'])
    ...
    def _product_find(self, product_template, product_attributes):
        if product_template:
            domain, cont = self._build_attributes_domain(product_template, product_attributes)
    ...
    def _check_duplicity(self):
        for product in self:
            ...
            for other_product in other_products:
                if (len(other_product.attribute_value_ids) == cont and other_product != product):
    ...
    def _check_configuration_validity(self):
        ...
        if self.env.context.get('creating_variants'):
            return
        for product in self:
            req_attrs = product.product_tmpl_id.attribute_line_ids.filtered(lambda a: a.required).mapped('attribute_id')
            errors = (req_attrs - product.attribute_value_ids.mapped('attribute_id'))
            if errors:
                raise exceptions.ValidationError(_("You have to fill the following attributes:\n%s") %
                                                 "\n".join(errors.mapped('name'))
                                                 )

    @api.model
    def create(self, vals):
        if (not vals.get('attribute_value_ids')[0][2] and vals.get('product_attribute_ids')):
            vals['attribute_value_ids'] = ((4, x[2]['value_id'])
                                           for x in vals.pop('product_attribute_ids') if x[2].get('value_id'))

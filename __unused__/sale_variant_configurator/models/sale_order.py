...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        for line in lines:
            product = product_obj._product_find(line.product_tmpl_id, line.product_attribute_ids, )
            if not product:
...
class SaleOrderLine(models.Model):
    ...
    def _onchange_product_tmpl_id_configurator(self):
        ...
        if self.product_tmpl_id.attribute_line_ids:
            ...
            pricelist = self.order_id.pricelist_id.with_context({
                'uom': self.product_uom.id,
                'date': self.order_id.date_order
            })
            if pricelist:
                price = pricelist.template_price_get(self.product_tmpl_id.id,
                                                     self.product_uom_qty or 1.0,
                                                     self.order_id.partner_id.id)
                if price:
                    self.price_unit = price[self.order_id.pricelist_id.id]
        # Update taxes

...
class AssignManualQuants(models.TransientModel):
    ...
    quants_lines = fields.One2many('assign.manual.quants.lines', 'assign_wizard', string='Quants')
    ...
    def assign_quants(self):
        ...
        self.pool['stock.quant'].quants_reserve(self.env.cr, self.env.uid, quants, move, context=self.env.context)
        return {}
    ...
    def default_get(self, var_fields):
        ...
        available_quants_ids = self.env['stock.quant'].search(
            ['|', ('location_id', '=', move.location_id.id),
             ...
             ('location_id.usage','=', 'internal'),       # dorin oare mai e necesar?
             ('product_id', '=', move.product_id.id),
             ('qty', '>', 0),
             ('reservation_id', '=', False)])
        available_quants = [{'quant': x.id, 'cost': x.cost} for x in available_quants_ids]
...
class AssignManualQuantsLines(models.TransientModel):
    ...
    assign_wizard = fields.Many2one('assign.manual.quants', string='Move',  required=True, ondelete="cascade")
    quant = fields.Many2one('stock.quant', string="Quant", required=True,  ondelete='cascade')
    
    cost = fields.Float( string='Price', readonly=True )

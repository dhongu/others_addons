...
class ProductConfigurator(models.AbstractModel):
    ...
    @api.multi
    @api.depends('product_attribute_ids', 'product_attribute_ids.value_id', 'product_id')
    def _compute_can_be_created(self):
        for rec in self:
            ...
            rec.can_create_product = not bool(
                ...
                len(filter(None, rec.product_attribute_ids.mapped('value_id'))))
    ...
    def _onchange_product_attribute_ids_configurator(self):
        ...
        domain, cont = product_obj._build_attributes_domain(self.product_tmpl_id, self.product_attribute_ids)
        self.product_id = False
        if cont:
        ...
        if not self.product_id:
            ...
            if 'partner_id' in self._fields:
                ...
                obj = self.env['product.attribute.value'].with_context(lang=self.partner_id.lang)
                values = obj.browse(self.product_attribute_ids.mapped('value_id').ids)
                obj = self.env['product.template'].with_context(lang=self.partner_id.lang)
    ...
    def _onchange_product_id_configurator(self):
        ...
        if self.product_id:
            ...
            if 'partner_id' in self._fields:
                # If our model has a partner_id field, language is got from it
                product = self.env['product.product'].with_context(lang=self.partner_id.lang).browse(self.product_id.id)
            self.name = self._get_product_description(product.product_tmpl_id, product, product.attribute_value_ids)
    ...
    def create_variant_if_needed(self):
        ...
        product_obj = self.env['product.product']
        product = product_obj._product_find(self.product_tmpl_id, self.product_attribute_ids)
        if not product:
            product = product_obj.create({
                'product_tmpl_id': self.product_tmpl_id.id,
                'attribute_value_ids': [(6, 0, self.product_attribute_ids.mapped('value_id').ids)],
            })
        self.product_id = product.id
        return product

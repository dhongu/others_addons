...
class ProductAttributeValue(models.Model):
    ...
    def create(self, vals):
        ...
        if 'template_for_attribute_value' in self.env.context:
            template = self.env['product.template'].browse(self.env.context['template_for_attribute_value'])
            line = template.attribute_line_ids.filtered(lambda x: x.attribute_id == attr_value.attribute_id)
